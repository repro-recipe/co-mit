...
const RichTextEditor: React.FC<RichTextEditorProps> = ({ value, onChange, placeholder, className, minHeight = '200px' }) => {
  ...
  return (
    <div className={`border-2 border-slate-300 rounded-md overflow-hidden bg-white transition-colors ${isFocused ? 'ring-2 ring-sky-500 border-sky-500' : ''} ${className}`}>
      <div className="flex items-center gap-1 p-2 bg-slate-50 border-b border-slate-200 flex-wrap">
        <div className="flex items-center gap-1 border-r border-slate-300 pr-2 mr-2">
             <select 
                className="bg-transparent text-sm font-bold text-slate-600 p-1 cursor-pointer outline-none"
      ...
      <div 
        ref={editorRef}
        contentEditable
        className="p-4 outline-none text-slate-800 overflow-y-auto"
        style={{ minHeight }}
        onInput={handleInput}
